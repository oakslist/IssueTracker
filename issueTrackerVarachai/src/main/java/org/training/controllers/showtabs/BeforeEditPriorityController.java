...
/**
 * Servlet implementation class BeforeEditPriorityController
 */

public class BeforeEditPriorityController extends AbstractBaseController {
	...
	protected void performTask(HttpServletRequest request,
		...
		int editPriorityId = Integer.parseInt(request.getParameter("hidden3"));
		
		try {
			// get type from db
			ITableDataDAOHib tableDataDAO = TableDataFactoryHib.getClassFromFactory();
			Priority priority = tableDataDAO.getPriorityById(editPriorityId);
			session.setAttribute(ServletConstants.JSP_EDIT_PRIORITY, priority);
			jump(ServletConstants.JUMP_EDIT_PRIORITY_PAGE, request, response);
